...
export-env {
    ...
    let virtual_env = r#'/Users/gimmingyu/Desktop/oz/kim&ham/new_passion_fruit/.venv'#
    let bin = r#'bin'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'new-passion-fruit-py3.13'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'new-passion-fruit-py3.13'#
    }
